...
def normalize_name_for_matching(name: str) -> str:
    ...
    prefixes = ['dr.', 'prof.', 'dott.',
                'dott.ssa', 'sig.', 'sig.ra', 'sig.na']
...
def is_partial_name_match(base_name: str, full_name: str) -> bool:
    """
    Check if base_name is a partial match of full_name.

    This handles cases where DTX has suffixes like "BIS", "TRIS", "II", "JR"
    that aren't present in PMS data.

    Args:
        base_name: The base name from PMS (shorter)
        full_name: The full name from DTX (potentially with suffixes)

    Returns:
        True if base_name is contained in full_name
    """
    if not base_name or not full_name:
        return False

    base_normalized = normalize_string(base_name)
    full_normalized = normalize_string(full_name)

    # Check if base name is a substring of full name
    return base_normalized in full_normalized and len(base_normalized) > 0
