...
class ClinicalMatchingController:
    ...
    def execute_matching_workflow(self, 
        ...
        try:
            ...
            print(f"Confidence threshold: {self.confidence_threshold:.1%}", file=sys.stderr)
            
            if output_file is None:
                print("Audit-only mode: No output file will be generated", file=sys.stderr)
            
            # Process DTX file
            stats = self.service.process_dtx_file(dtx_file, pms_lookup, output_file)
            
            # Generate reports
            if generate_audit:
                manual_review_queue = self.service.get_manual_review_queue()
                self.reporting.generate_audit_report(stats, manual_review_queue)
